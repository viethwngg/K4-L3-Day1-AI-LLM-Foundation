...
OPENAI_MODEL = os.getenv("LAB_MODEL", "gemini-3.8-flash")
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    headers = [
        ...
        f"{OPENAI_MODEL} Latency",
        f"{OPENAI_MINI_MODEL} Latency",
    ]
    rows = [headers]
...
if __name__ == "__main__":
    print(f"=== So sánh {OPENAI_MODEL} vs {OPENAI_MINI_MODEL} ===")
    result = compare_models(
        "Giải thích khác biệt giữa temperature và top_p trong một câu."
    )
    print(f"{OPENAI_MODEL}_response: {result['gpt4o_response']}")
    print(f"{OPENAI_MINI_MODEL}_response: {result['mini_response']}")
    print(f"{OPENAI_MODEL}_latency: {result['gpt4o_latency']}")
    print(f"{OPENAI_MINI_MODEL}_latency: {result['mini_latency']}")
    print(f"{OPENAI_MODEL}_cost_estimate: {result['gpt4o_cost_estimate']}")
